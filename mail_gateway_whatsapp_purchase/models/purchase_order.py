...
class PurchaseOrder(models.Model):
    ...
    def _whatsapp_phone_field_name(self):
        """Return the vendor phone field used to resolve the channel."""
        self.ensure_one()
        if not self.partner_id:
            raise UserError(
                self.env._(
                    "You must set a vendor before sending the order by WhatsApp."
                )
            )
        return "mobile" if self.partner_id.mobile else "phone"

    def _whatsapp_get_channel(self, field_name, gateway):
        """Resolve the WhatsApp channel from the vendor phone number.
    ...
    def _whatsapp_get_attachments(self):
        """Return the request for quotation PDF to be sent with the message.
    ...
    def action_send_whatsapp(self):
        """Open the WhatsApp composer to send the order to the vendor."""
        self.ensure_one()
